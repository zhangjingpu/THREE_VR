...
function ScrollPic(scrollContId,arrLeftId,arrRightId,dotListId){
    ...
    this.initialize = function(){
        ...
        this.stripDiv.style.width="10000%";
        ...
        if(this.arrLeftId){
            ...
            if(this.arrLeftObj){
                ...
                sina.addEvent(this.arrLeftObj, "touchstart", Function("ScrollPic.childs[" + this.ID + "].rightMouseDown()")); //手指放到屏幕上时触发
                sina.addEvent(this.arrLeftObj, "touchend", Function("ScrollPic.childs[" + this.ID + "].rightEnd()"));//手指离开屏幕时触发
            }
        };
        if(this.arrRightId){this.arrRightObj=sina.$(this.arrRightId);
            if(this.arrRightObj){
                ...
                sina.addEvent(this.arrLeftObj, "touchstart", Function("ScrollPic.childs[" + this.ID + "].leftMouseDown()")); //手指放到屏幕上时触发
                sina.addEvent(this.arrLeftObj, "touchend", Function("ScrollPic.childs[" + this.ID + "].leftEnd()"));//手指离开屏幕时触发
            }
        };
        ...
        if(this.dotListId){
            this.dotListObj=sina.$(this.dotListId);if(this.dotListObj){
                ...
                for(i=0;i<pages;i++){tempObj=document.createElement("span");
                    ...
                    tempObj.title="��"+(i+1)+"ҳ";sina.addEvent(tempObj,"click",Function("ScrollPic.childs["+this.ID+"].pageTo("+i+")"))
                }
            }
        };
        if(this.autoPlay){
    ...
    this.rightEnd=function(){if(_state!="floating"){
        return
    };
    ...
    this.move=function(num,quick){
        ...
        num-=thisMove;if(Math.abs(num)==0){
            _state="ready";
            if(this.autoPlay){
                this.play()};this.accountPageIndex();
            return
        }else{
            this.accountPageIndex();
