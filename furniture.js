...
var XML_D = {
    ...
    init : {
        ...
        initURL : {
            /**加载xml文件的访问路径
             * 单个柜体 ：http://www.itthome.com/OnlineProducts/Products/UnitCabinet/xml/L0-00001.cxml
             * 组合柜 ：http://www.itthome.com/OnlineProducts/Products/GroupCabinet/xml/Z-Y-X-J-00064.cxml
             * id = K0-00002 ：文件名称
             * type ：路径类型
             * 首页：  http://www.itthome.com/3DCloudDesign/WebMain.aspx
             *         http://192.168.0.134:2899/Web3D/furniture.html **/
            //url: "xml/网上产品库/组合柜/Z-Y-X-J-00074.xml",
            //url: "xml/网上产品库/单元柜/N0-00002.xml",
            url: "xml/本地产品库/单元柜/掩门下柜一区间.xml",
            //url: "xml/本地产品库/组合柜/书柜-柜体4.xml",

            xmlUrl: "xml/",

            //加载图片的文件夹位置 http://www.itthome.com/OnlineProducts/Products/ProductMaterial/jpg/    img/maps/
            imgUrl: "img/maps/",
        ...
        initFun : {
            start : function(){
                /**回调函数1
                 * this接收回调函数的数据
                 * XML_D.Fun.transformXMLToJson(this);
                 *
                 * 回调函数2：
                 * 在function中声明参数 function(xmlhttp){} */
                //XML_D.Utils.XMLHttpRequestObjectPool.sendReq("GET",XML_D.init.initURL.url,"",{},function(xmlhttp){
                //    XML_D.Fun.transformXMLToJson(xmlhttp);
                //    XML_D.init.initThree.threeStart();
                //});

                var loader = new THREE.XHRLoader();
                loader.load ( XML_D.init.initURL.url, function(data){
                    var xmlDom = new DOMParser().parseFromString(data, "text/xml");
                    XML_D.XML.transformXMLToJson_combination(xmlDom);
                    //设置title
                    $("title").html(XML_D.init.initData.scene.ComposeList[0].Name);
                    XML_D.init.initThree.threeStart();
                });

                //$.ajax({
                //    url: XML_D.init.initURL.url,
                //    success: function (data) {
                //        XML_D.XML.transformXMLToJson_combination(data);
                //        XML_D.init.initThree.threeStart();
                //    }
                //});
            },
    ...
    changeMaterial : function(alt){
        ...
        if(XML_D.DtaFields.CurrentGeometry.geometry){
            ...
            if (XML_D.init.UIDate.checked) {
                //更换贴图的材质
                updateMaterial(XML_D.DtaFields.CurrentGeometry.geometry,true);
         ...
         * type : true. 表示单个板件
         *        false. 表示整体柜 默认 **/
        function updateMaterial(mesh ,type){
            if(type){
                setMaterial(mesh);
            }else{
                //判断是否是独立花色
...
XML_D.XML = {
    ...
    transformXMLToJson_combination : function(xmlhttp){
        ...
        var scene = XML_D.init.initData.scene;
